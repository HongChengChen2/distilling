...
def train_epoch(train_loader, big_model, small_model, T, criterion, optimizer, epoch, loss_weight=0.5):
    big_model.eval()
    big_model.cuda()
    small_model.train()
    small_model.cuda()
    ...
    for inp, class_target in pbar:
        ...
        soft_logits_small = F.softmax(logits_small * 1.0/T ,dim = 1 ) 
        soft_logits_big = F.softmax(logits_big *1.0/T ,dim =1 ).detach()
